...
class SqliteDBConnection:
    def __init__(self):
        self.database_path = os.path.join(os.getcwd() + '/files/deadlines.db')
...
class Deadlines:
    ...
    def download(self, guild):
        ...
        self.get_time()
        file = os.getcwd() + '/files/' + str(guild) + '/icalexport.ics'
        ...
        handler.info('Updated calendar file for ' + guild)
    ...
    def get(self, guild):
        ...
        try:
            with open(os.getcwd() + '/files/' + guild + '/icalexport.ics', 'rb') as g:
                ...
                for i in range(len(dates)):
                    ...
                    self.database.modify_value(query, guild, dates[i], courses[i], contents[i])
        except FileNotFoundError:
            handler.error('No calendar file found for ' + guild)
...
class Moodle(commands.Cog):
    ...
    def check_setup(self, ctx):
        ...
        result = self.database.execute_query(query, str(ctx.guild))
        try:
            if str(result[0][0]) == str(ctx.guild):
                return True
        except IndexError:
            return False
    ...
    async def setup(self, ctx, url):
        ...
        if self.check_setup(ctx) is False:
            ...
            self.database.modify_value(query, str(ctx.guild), str(webhook_url), str(url))
    ...
    async def reset(self, ctx):
        ...
        if self.check_setup(ctx) is True:
            ...
            path = os.getcwd() + '/files/' + str(ctx.guild)
            if os.path.exists(path):
                shutil.rmtree(path)
            query = "DELETE FROM GUILDS WHERE guild_name = ?"
            self.database.modify_value(query, str(ctx.guild))
            query = "DELETE FROM DEADLINES WHERE guild = ?"
            self.database.modify_value(query, str(ctx.guild))
    ...
    async def force_update(self, ctx):
        ...
        if self.check_setup(ctx) is True:
            ...
            async with ctx.typing():
                self.deadlines.download(str(ctx.guild))
    ...
    async def deadlines(self, ctx):
        ...
        if self.check_setup(ctx) is True:
            await ctx.message.delete()
            embed = self.deadlines.embed(str(ctx.guild))
